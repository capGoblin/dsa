...
public class Heap {
  // A Tree which is (Btree -> BHeap)
  // Complete(filled from left to right)
  // HeapProperty(every node >= its children)

  //not support lookup by values
  private int[] items = new int[10];
  private int size;

  // O(log n) - at worst case have to move to root to h
  ...
  private void bubbleUp() { // O(log n) or O(h)
    var index = size - 1;
