...
public class Tree { // a graph without cycles with root node
  // BST (balanced tree -> height(left) - height(right) <= 1)
  // lookUp O(log n) or O(h) - narrowing each search by half 
  // insert O(log n) 
  // delete O(log n) 
  // if BST not structured properly or unbalanced, could reach O(n) for Lookup, insert, and delete -> solved by AVLTrees(self-balancing trees)
  private class Node {
    private int value;
  ...
  // O(log n) - BST(follows left<root<right)
  public int min() {
    if (root == null)
  ...
  public ArrayList<Integer> getNodesAtDistance(int distance) { //or getNodesAtDepth
